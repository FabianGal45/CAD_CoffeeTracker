...
sudo docker run -e RAILS_ENV=$production  -e RAILS_MASTER_KEY=$RAILS_MASTER_KEY -e SECRET_KEY_BASE=$SECRET_KEY_BASE -p 3000:3000 -b tcp://0.0.0.0:3000 -d --name $CONTAINER_NAME $IMAGE_NAME 